...
import 'widgets/object_sections.dart';
import 'widgets/object_settings_sheet.dart';

/// Model 02 — object detection and tracking.
...
class _ObjectScreenState extends State<ObjectScreen> with WidgetsBindingObserver {
  final ObjectLab _lab = ObjectLab();
                  ...
                  Expanded(
                    child: ListView(
                      ...
                      children: [
                        VisionViewport(
                          camera: _lab.camera,
                          painter: ObjectOverlayPainter(frame: _lab.frame),
                          sourceSize: _lab.frame.sourceSize,
                          isStill: _lab.isStill,
                          still: _lab.still,
                          latencyMs: _lab.latencyMs,
                          count: _lab.objectCount,
                          noun: 'object',
                          accent: Brand.amber,
                        ),
                        ObjectSections(lab: _lab),
                      ],
                    ),
                  ),
                  _controlBar(),
  ...
  Widget _header() {
    final (label, color) = switch (_lab.camera.status) {
      RigStatus.running => (_lab.isStill ? 'Photo' : 'Live', Brand.amber),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text('Model 02', style: Typo.eyebrow.copyWith(fontSize: 11)),
                const SizedBox(height: 2),
                Text('Object Detection', style: Typo.navTitle),
              ],
            ),
          ),
          Container(
          ...
          child: Row(
            children: [
              ...
              DeckButton(
                ...
                label: 'Settings',
                accent: Brand.amber,
                onTap: () => ObjectSettingsSheet.show(context, _lab),
              ),
            ],
          ),
        ),
